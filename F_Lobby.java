...
public class F_Lobby {
    ...
    */
    ...
    Em seguida, chamamos o método emparelharUsuarios(usuario) para procurar um oponente compatível para o jogador recém-adicionado.*/
    ...
    /*método responsável por procurar um oponente compatível para um jogador recém-adicionado ao lobby.

    Ele itera sobre a lista de usuários (usuarios) e verifica se o jogador é diferente do oponente, se eles têm o mesmo nível e se estão jogando na mesma modalidade.

    Quando um oponente compatível é encontrado, chamamos o método criarPartida para iniciar uma partida entre esses dois jogadores.

    O return é usado para interromper a busca por oponentes após o emparelhamento bem-sucedido.

    Se nenhum oponente compatível for encontrado, printa a mensagem "Aguardando oponente...". */
    ...
    /*Metodo cria uma partida entre dois jogadores, desde que essas condições sejam atendidas:

    -Ambos os jogadores (jogador1 e jogador2) não sejam nulos ( eles tem q existir).
    -Ambos os jogadores tenham um baralho disponível (ou seja, seus baralhos não são nulos( ele tem q existir)).

    Se essas condições não forem atendidas, o método printa mensagens de erro apropriadas.  */
    ...
    //Metodo permite que um objeto do tipo A_Usuario (que representa um jogador) adicione seu baralho à lista de usuários no lobby.
